# Weighted - Nondominated Sorting Genetic Algorithm II (NSGA-II) An multi-objective algorithm
# Constrain NSGA-II using the information of a dft point
# Search the region that can simutaneously improve all objectives
from __future__ import division, print_function, absolute_import
import sampling
import numpy as np
import copy

def optimization(model, nInput, nOutput, xlb, xub, dft, pop, gen, \
                 crossover_rate = 0.9, mu = 20, mum = 20, weight = 0.001):
    ''' Weighted - Nondominated Sorting Genetic Algorithm II, An multi-objective algorithm
    ...
    x, y, rank, crowd = sortMO_W(x, y, nInput, nOutput, dft, weight)
    ...
    for i in range(gen):
        ...
        population_para, population_obj = \
            remove_worst(population_para, population_obj, pop, nInput, nOutput, dft, weight)
        bestx = population_para.copy()
        besty = population_obj.copy()
    return bestx, besty, x, y

def sortMO_W(x, y, nInput, nOutput, dft, weight = 0.001):
    ...
    for k in range(rmax):
        rankidx = (rank == k)
        D = weighted_crowding_distance(y[rankidx,:], dft, weight)
        idxd = D.argsort()[::-1]
...
def weighted_crowding_distance(Y, dft, weight = 0.001):
...
def remove_worst(population_para, population_obj, pop, nInput, nOutput, dft, weight):
    ''' remove the worst individuals in the population '''
    population_para, population_obj, rank, crowd = \
        sortMO_W(population_para, population_obj, nInput, nOutput, dft, weight)
